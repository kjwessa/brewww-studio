...
async function getPageData({ slug }: { slug: string }) {
  ...
  // Fetch technologies
  const technologiesResponse = await fetch(`${process.env.NEXT_PUBLIC_SERVER_URL}/api/technologies`, {
...
export default async function LocationPage({ params }: { params: { slug: string } }) {
  const { technologies, location, faqs } = await getPageData({ slug: params.slug });
